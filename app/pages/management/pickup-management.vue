...
function formatIcon(icon?: string) {
  if (!icon) return 'i-lucide-trash-2'
  if (icon.startsWith('i-lucide-')) return icon
  if (icon.startsWith('lucide:')) return 'i-' + icon
  return `i-lucide-${icon}`
}

const filteredDisposable = computed(() => {
  const q = searchDisposable.value.toLowerCase().trim()
  if (!q) return disposableTypes.value
  return disposableTypes.value.filter(d => 
...
const filteredQuantity = computed(() => {
  const q = searchQuantity.value.toLowerCase().trim()
  if (!q) return estimatedQuantities.value
  return estimatedQuantities.value.filter(item => 
...
const addDisposableModalRef = ref()
...
const editDisposableModalRef = ref()
...
const editQuantityModalRef = ref()
...
<template>
  <div style="font-family:'Manrope',sans-serif;display:flex;flex-direction:column;gap:28px">
            ...
            <tr v-for="item in filteredDisposable" :key="item.id" style="border-bottom:1px solid #f0f0f0">
              <td style="padding:16px 24px">
                <div style="width:36px;height:36px;border-radius:10px;background:#f9fafb;display:flex;align-items:center;justify-content:center">
                  <UIcon :name="formatIcon(item.icon)" style="width:18px;height:18px;color:#6b7280" />
                </div>
              </td>
              ...
              <td style="padding:16px 24px;text-align:right">
                <div style="display:flex;gap:8px;justify-content:flex-end">
                  ...
                  <button @click="openDeleteDisposable(item)" style="display:flex;align-items:center;gap:6px;background:#fef2f2;color:#ef4444;border:none;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:600;font-family:'Manrope',sans-serif;cursor:pointer">
                    <UIcon name="i-lucide-trash-2" style="width:12px;height:12px" />
                    Delete
                  </button>
                </div>
              </td>
    ...
    <div v-if="activeTab === 'quantity'" style="display:flex;flex-direction:column;gap:24px">
      ...
      <div style="background:#fff;border-radius:16px;border:1px solid #f0f0f0;overflow:hidden">
        ...
        <div v-if="filteredQuantity.length === 0" style="padding:60px 24px;text-align:center">
          <UIcon name="i-lucide-ruler" style="width:40px;height:40px;color:#d1d5db;margin-bottom:12px" />
